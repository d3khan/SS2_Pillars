...
#define MATERIAL_MASK_THRESHOLD (0.3)
#define CUSTOM_VERTEX_INTERPOLATORS_COUNT (0)
#define MATERIAL_OPACITY_THRESHOLD (0.12)
// Motion Vectors: Defines
// Lightmap: Defines
#define CAN_USE_LIGHTMAP 1
// Deferred Shading: Defines
...
Material GetMaterialVS(MaterialInput input)
{
	...
	material.PositionOffset = float3(0, 0, 0);
	material.TessellationMultiplier = 4.0f;
	return material;
}
...
{
	Material material = (Material)0;
	material.WorldDisplacement = float3(0, 0, 0);
	return material;
}
...
Material GetMaterialPS(MaterialInput input)
{
	...
	material.TangentNormal = float3(0, 0, 1.0);
	float4 local0;
	float4 local1 = In1.Sample(SamplerLinearWrap, input.TexCoord.xy);
{
// Convert old Specular value to a new range
local0.x = min(float4(local1.z, local1.z, local1.z, local1.z).x * 0.5f, 0.6f);}
	material.Color = local0.xyz;
	...
	material.SubsurfaceColor = float3(0, 0, 0);
	material.Mask = 1.0;
	material.TangentNormal *= input.TwoSidedSign;
	material.TangentNormal = normalize(material.TangentNormal);
	material.WorldNormal = normalize(TransformTangentVectorToWorld(input, material.TangentNormal));
	material.Metalness = saturate(material.Metalness);
	material.Roughness = max(0.04, material.Roughness);
